...
tape('selection.data(fromState(selector)) gets state from the provided store', function (test) {
  var state = { foo: [0, 1, 2] };
  var document = jsdom('');
...
tape('selection.data(fromState(selector)) calls the selector in the context of the selected element', function (test) {
  var result;
...
tape('selection.datum(fromState(selector)) gets state from the provided store', function (test) {
  var state = { foo: {} };
  var document = jsdom('<div></div>');
  var sel = d3.select(document.body)
    .call(provide(storeOf(state)))
...
tape('selection.datum(fromState(selector)) calls the selector in the context of the selected element', function (test) {
  var result;
  var document = jsdom('<div></div>');
  var el = document.querySelector('div');
