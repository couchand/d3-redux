...
tape('selection.on(type, dispatch(actionCreator)) dispatches on event', function(test) {
  var document = jsdom('<div></div>');
  var div = document.querySelector('div');
  var actions = [];
  var sel = d3.select(document.body)
    .call(provide({ dispatch: function (action) { actions.push(action);} }))
    .select('div')
...
tape('selection.on(type, dispatch(actionCreator) passes the actionCreator data, index and group', function(test) {
